...
def _run_methods(*, directory: Path, box, generator=None, oracle=None, evaluator=None, save_gp=True, metadata: dict, target_id: str,
    ...
    if not (initial == 0 or 2 <= initial <= budget) or budget < 1 or not methods or len(set(methods)) != len(methods) or set(methods)-set(AVAILABLE_METHODS):
    ...
    for method in methods:
        ...
        try:
            report = _run_method(state, method, context, box, generator, oracle, evaluator,
                                 save_gp, initial_x, initial_results, initial_hash, controller,
                                 controller_identity, acquisition_settings, fit_maxiter)
...
def _shared_initial(directory, box, generator, oracle, evaluator, initial, seed, context):
    if not initial:
    ...
    if not torch.equal(initial_x,design) or len(initial_results)!=initial:
    ...
    shared.close()
    return initial_x, initial_results, initial_hash


def _run_method(state, method, context, box, generator, oracle, evaluator, save_gp,
                initial_x, initial_results, initial_hash, controller, controller_identity,
                acquisition_settings, fit_maxiter):
    ...
    while state.remaining and state.get_setting("stop_decision") is None:
        ...
        if agent_info is not None:
            ...
            agent_info.update(trial_id=result["trial_id"],objective=result["objective"],
                improvement_after_override=max(0.0,result["objective"]-old_best) if result["valid"] and old_best is not None and agent_info["overrode_dsp_suggestion"] else None)
